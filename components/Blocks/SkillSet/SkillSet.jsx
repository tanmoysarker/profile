...
export default function SkillSet() {
    ...
    return (
        <>
                ...
                <div className={styles.grid}>
                    ...
                    <div className={styles.circularCarouselWrapper}>
                        ...
                        <div className={styles.circularCarousel}>
                            ...
                            {/* Optimized for exactly 18 skills */}
                            {Skills.map((skill, index) => {
                                // For 18 skills, use 3 circles with 6 items each
                                const itemsPerCircle = 6;
                                const circleIndex = Math.floor(index / itemsPerCircle);
                                const positionInCircle = index % itemsPerCircle;

                                // Three circles with generous spacing
                                const radii = [140, 200, 260];
                                const radius = radii[circleIndex];

                                // For 6 items per circle, use exactly 60 degrees between each (360/6)
                                // But offset each circle by 30 degrees to prevent alignment
                                const baseAngle = (positionInCircle * 60); // 60 degrees between items
                                const circleOffset = circleIndex * 30; // Offset each circle by 30 degrees
                                const currentAngle = baseAngle + circleOffset;

                                // Icon sizes - smaller on outer circles
                                const iconSizes = [76, 88, 60];
                                const iconSize = iconSizes[circleIndex];

                                // Circle classes
                                const circleClasses = ['', styles.outerCircle, styles.extremeCircle];
                                const circleClass = circleClasses[circleIndex] || '';

                                return (
                                    <div
                                        key={index}
                                        className={`${styles.box} ${activeIndex === index ? styles.isActive : ''} ${circleClass}`}
                                        style={{
                                            transform: `rotate(${currentAngle}deg) translate(${radius}px) rotate(-${currentAngle}deg)`,
                                            zIndex: 3 - circleIndex // Inner circles on top
                                        }}
                                    >
                                        <Image
            ...
            <Ticker words={['friendly', 'responsiveness', 'attention to details', 'performance']}></Ticker>
        </>
    )
}
